...
#include "qcc74x_adc.h"  // [NEW] Added for MQ-135
#include "qcc74x_gpio.h" // [NEW] For Pin Config
...
#define DHT_PIN         GPIO_PIN_10   // DHT11 Data Pin
...
int main(void)
{
    ...
    while (1) {
        ...
        uint32_t len1 = strlen(line1);
        uint32_t len2 = strlen(line2);
        ...
        qcc74x_mtimer_delay_us(10); // Small gap between packets
        qcc74x_spi_poll_exchange(spi0, &header, NULL, 1);
        qcc74x_spi_poll_exchange(spi0, line1, rx_dummy, len1);
        qcc74x_gpio_set(gpio, SPI_CS_PIN);
        printf("SENT DATE\r\n");


        qcc74x_mtimer_delay_ms(10); // Small gap between packets

        qcc74x_gpio_reset(gpio, SPI_CS_PIN);
        qcc74x_mtimer_delay_us(10); // Small gap between packets
        qcc74x_spi_poll_exchange(spi0, &header, NULL, 1);
        qcc74x_spi_poll_exchange(spi0, line2, rx_dummy, len2);
        qcc74x_gpio_set(gpio, SPI_CS_PIN);
        printf("SENT TIME\r\n");
        ...
        qcc74x_gpio_set(gpio, SPI_CS_PIN);
        printf("SENT TEMP,HUMIDITY AND AQI\r\n");
